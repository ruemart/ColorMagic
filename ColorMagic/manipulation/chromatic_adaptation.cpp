...
color_space::color_base * color_manipulation::color_adjustments::von_kries_adaptation(color_space::color_base * color, color_space::rgb_color_space_definition * target_color_space)
{
	...
	auto transformed_components = (calculate_transform_matrix(tmp_color->get_rgb_color_space()->get_white_point(), target_color_space->get_white_point(), m_von_kries, m_inverted_von_kries)) * tmp_color->get_component_vector();
...
matrix<float> color_manipulation::color_adjustments::calculate_transform_matrix(color_space::white_point * source_wp, color_space::white_point * dest_wp, matrix<float> mat, matrix<float> inverted_mat)
{
	// Create scaled white point vectors
	auto scaled_source_wp = m_von_kries * std::vector<float> { source_wp->get_tristimulus().begin(), source_wp->get_tristimulus().end() };
	auto scaled_dest_wp = m_von_kries * std::vector<float> { dest_wp->get_tristimulus().begin(), dest_wp->get_tristimulus().end() };
	...
	return inverted_mat * wp_matrix * mat;
}
