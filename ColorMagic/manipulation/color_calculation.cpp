...
color_space::rgb_truecolor * color_manipulation::color_calculation::mix(color_space::rgb_truecolor * color1, color_space::rgb_truecolor * color2, float weight)
{
	...
	auto r = std::sqrtf(weight * std::powf(color1->red(), 2.f) + (1.f - weight) * std::powf(color2->red(), 2.f));
	auto g = std::sqrtf(weight * std::powf(color1->green(), 2.f) + (1.f - weight) * std::powf(color2->green(), 2.f));
	auto b = std::sqrtf(weight * std::powf(color1->blue(), 2.f) + (1.f - weight) * std::powf(color2->blue(), 2.f));
	auto a = std::sqrtf(weight * std::powf(color1->alpha(), 2.f) + (1.f - weight) * std::powf(color2->alpha(), 2.f));
...
color_space::grey_truecolor * color_manipulation::color_calculation::mix(color_space::grey_truecolor * color1, color_space::grey_truecolor * color2, float weight)
{
	...
	auto v = std::sqrtf(weight * std::powf(color1->grey(), 2.f) + (1.f - weight) * std::powf(color2->grey(), 2.f));
	auto a = std::sqrtf(weight * std::powf(color1->alpha(), 2.f) + (1.f - weight) * std::powf(color2->alpha(), 2.f));
...
color_space::cmyk * color_manipulation::color_calculation::mix(color_space::cmyk * color1, color_space::cmyk * color2, float weight)
{
	...
	auto c = weight * color1->cyan() + (1.f - weight) * color2->cyan();
	auto m = weight * color1->magenta() + (1.f - weight) * color2->magenta();
	auto y = weight * color1->yellow() + (1.f - weight) * color2->yellow();
	auto k = weight * color1->black() + (1.f - weight) * color2->black();
...
color_space::hsv * color_manipulation::color_calculation::mix(color_space::hsv * color1, color_space::hsv * color2, float weight)
{
	...
	vector1[0] = weight * vector1[0] + (1.f - weight) * vector2[0];
	vector1[1] = weight * vector1[1] + (1.f - weight) * vector2[1];
	vector1[2] = weight * vector1[2] + (1.f - weight) * vector2[2];
...
color_space::xyz * color_manipulation::color_calculation::mix(color_space::xyz * color1, color_space::xyz * color2, float weight)
{
	...
	auto x = weight * color1->x() + (1.f - weight) * color2->x();
	auto y = weight * color1->y() + (1.f - weight) * color2->y();
	auto z = weight * color1->z() + (1.f - weight) * color2->z();
...
{
	return color_manipulation::color_converter::to_lab(color_manipulation::color_calculation::mix(color_manipulation::color_converter::to_xyz(color1), color_manipulation::color_converter::to_xyz(color2), weight));
}

float * color_manipulation::color_calculation::convert_to_vector(float hue, float saturation, float third_component)
{
	auto x = cosf(hue / 180 * M_PI) * saturation;
	auto y = sinf(hue / 180 * M_PI) * saturation;
...
{
	auto x = std::atan2(color[1], color[0]) * 180 / M_PI;
